...
const getActiveFlag = (item) =>
  Boolean(
    item.active ?? item.Active ?? item.isActive ?? item.IsActive ?? false,
  );

const mapScheduleToEvent = (item) => {
  const start =
    item.startDate && item.startTime
      ? `${item.startDate.split("T")[0]}T${item.startTime}`
      : item.startDate;

  const end =
    item.startDate && item.endTime
      ? `${item.startDate.split("T")[0]}T${item.endTime}`
      : item.startDate;
...
const CalendarComponent = () => {
  ...
  const updateEventActiveState = (eventId, active) => {
    ...
    setEvents((prev) =>
      prev.map((ev) =>
        ev.id === idStr
          ? { ...ev, extendedProps: { ...ev.extendedProps, active } }
          : ev,
      ),
    );
  };
